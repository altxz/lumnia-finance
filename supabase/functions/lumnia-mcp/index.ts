...
import { auth, defineMcp, setLogLevel } from "npm:@lovable.dev/mcp-js@0.26.3";
  ...
  handler: safeHandler("update_transaction", async (input, ctx) => {
    ...
    try {
      ...
      if (row.is_recurring && scope !== "single") {
        ...
        const { error: cleanupError } = await sb.from("expenses").delete().eq("user_id", userId).eq("description", row.description).eq("type", row.type).eq("is_recurring", false).eq("is_paid", false).gte("date", cutoff);
...
function computeMonthCashFlow(effectiveMonthExpenses, isCreditCardPayment, investmentWalletIds = []) {
  ...
  return effectiveMonthExpenses.reduce((sum, expense) => {
    ...
    if (expense.type === "income") return sum + transactionAmount(expense.value);
    if (expense.credit_card_id) return sum;
    if (isCreditCardPayment(expense)) return sum;
    return sum - transactionAmount(expense.value);
  }, 0);
}
function buildDailyBalanceMap({
  ...
  monthExpenses.forEach((expense) => {
    ...
    nonCcFlowByDay[expense.date] += expense.type === "income" ? amount : -amount;
  });
...
function computeProjectedMonthResult({
  ...
  const totalIncome = nonTransfers.filter((expense) => expense.type === "income").reduce((sum, expense) => sum + transactionAmount(expense.value), 0) + investmentInflow;
  const debitOnly = nonTransfers.filter(
    (expense) => expense.type !== "income" && !expense.credit_card_id && !isCreditCardPayment(expense)
  );
  const debitExpense = debitOnly.reduce((sum, expense) => sum + transactionAmount(expense.value), 0) + investmentOutflow;
  ...
  const cardPurchasesTotal = purchasesInMonth.reduce((sum, p) => sum + transactionAmount(p.value), 0);
  ...
  debitOnly.forEach((expense) => {
    byCategory[expense.final_category] = (byCategory[expense.final_category] || 0) + transactionAmount(expense.value);
  });
  purchasesInMonth.forEach((purchase) => {
    const key = purchase.final_category || "outros";
    byCategory[key] = (byCategory[key] || 0) + transactionAmount(purchase.value);
  });
...
// src/lib/mcp/index.ts
var projectRef = "ulszjqppxceqbeoyyqbb";
setLogLevel("info");
var mcp_default = defineMcp({
  name: "lumnia-mcp",
  title: "Lumnia",
  version: "1.0.0",
  instructions: "MCP privado do Lumnia para gest\xE3o financeira pessoal. Valores em BRL, meses no formato YYYY-MM e datas YYYY-MM-DD. Consultas: search, fetch, list_transactions, month_summary, month_transactions, compare_months, list_budgets, list_categories, list_wallets, list_credit_cards e invoice_details. Opera\xE7\xF5es: create_transaction, update_transaction, delete_transaction, set_transaction_paid, create_transfer, pay_invoice, upsert_budget, create_budget, update_budget, delete_budget, manage_wallet, manage_category, create_category, update_category, delete_category, manage_project e investments. Antes de qualquer escrita, confirme a inten\xE7\xE3o e repita o impacto financeiro, data e conta envolvidos. Em recorr\xEAncias, parcelamentos, faturas, resgates ou exclus\xF5es, explique o alcance exato e obtenha confirma\xE7\xE3o expl\xEDcita antes de chamar a ferramenta. Nunca invente IDs, saldos, carteiras, cart\xF5es, categorias ou projetos. Quando houver ambiguidade, use as ferramentas de listagem ou pesquisa antes de alterar dados.",
