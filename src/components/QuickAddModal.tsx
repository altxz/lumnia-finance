          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 py-4 text-sm font-bold transition-colors ${
              isExpense
                ...
                : 'bg-foreground/[0.04] text-muted-foreground hover:bg-foreground/10'
            }`}
          >
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-foreground/[0.04] border border-border">
                <button type="button" onClick={() => { setPaymentMethod('debit'); setCreditCardId(''); setInstallments('1'); }}
                  className={`rounded-lg py-2 text-xs font-semibold transition-all ${paymentMethod === 'debit' ? 'bg-foreground/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                  💳 Débito
                </button>
                <button type="button" onClick={() => setPaymentMethod('credit')}
                  className={`rounded-lg py-2 text-xs font-semibold transition-all ${paymentMethod === 'credit' ? 'bg-foreground/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                    ...
                    <div className="space-y-1.5">
                      <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-foreground/[0.04] border border-border">
                        <button type="button" onClick={() => setInstallmentValueType('total')}
                          className={`rounded-lg py-1.5 text-xs font-semibold transition-all ${installmentValueType === 'total' ? 'bg-foreground/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                          Valor Total
                        </button>
                        <button type="button" onClick={() => setInstallmentValueType('per_installment')}
                          className={`rounded-lg py-1.5 text-xs font-semibold transition-all ${installmentValueType === 'per_installment' ? 'bg-foreground/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                          Valor da Parcela
                        </button>
                      </div>
