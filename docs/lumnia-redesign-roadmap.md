...
- Etapa 8: escopo implementado em 30/08/2026 (bottom sheets, carregamento escalonado, estado offline/retry). TypeScript, lint crítico, 97 testes e build web passaram; validado no navegador (light, alternando `navigator.onLine`). Falta a validação física no Galaxy S22 Ultra exigida pela regra de conclusão — etapa fica como "implementada, validação física pendente" até isso ser feito.
- Etapa 9: pendente, sem início.
...
- Próximo checkpoint: gerar um novo build Android e validar fisicamente no Galaxy S22 Ultra o escopo da Etapa 8 (bottom sheets, carregamento escalonado, offline/retry), depois seguir para a auditoria final (Etapa 9). Publicar a conta do Google Play Console continua fora de escopo por decisão do usuário.
...
- [x] Carregamento escalonado. Criado `StaggerItem` (`src/components/ui/stagger.tsx`), que revela cada bloco com um pequeno atraso proporcional à posição (`animate-in fade-in slide-in-from-bottom-2`, respeita `prefers-reduced-motion`). Aplicado ao Dashboard (5 seções), Análises (visão geral, insights e as 4 seções secundárias) e Histórico (cabeçalho e cartão de transações). Validado visualmente sem quebra de layout.
- [x] Estado offline e retry. Criado hook único `useOnlineStatus` (`src/hooks/useOnlineStatus.ts`, extraído da lógica já existente em Histórico) e componente `OfflineBanner` (`src/components/ui/offline-banner.tsx`) reaproveitando `StatePanel tone="offline"`. Ligado em Dashboard, Orçamento, Projetos, Categorias, Detalhes de categoria, Análises, Carteira e Investimentos — nas duas últimas isso exigiu capturar erros que antes eram descartados nos fetches (`fetchWallets`/`fetchCards`) e expor o `error` já disponível em `useInvestments`. Todo painel de erro existente agora distingue offline de erro genérico (tom, título e descrição mudam conforme `isOnline`). Validado no browser alternando `navigator.onLine` em Dashboard, Carteira, Investimentos e Categorias — banner e dados em cache exibidos corretamente.
